...
/**
  ******************************************************************************
  * @brief	Prints a number to the LCD screen.
  * @param	Number That has to be converted.
						Number of digits precession requried.
  * @retval	None
  ******************************************************************************
  */
void lcd16x2_PutNum(float Num,int decmi)
{
	char str[decmi];
	snprintf(str,decmi+1,"%f",Num);
	lcd16x2_puts(str);
}
...
/**
  ...
  ******************************************************************************
  */
void lcd16x2_clear_line(void)
{
	uint8_t location = lcd16x2_getxy();
	if(location < 0x40){ //This means the current loaction is First line
		lcd16x2_gotoxy(0,0);
		lcd16x2_puts("                ");
	}
	else{																//This means the current loaction is Second line
		lcd16x2_gotoxy(0,1);
		lcd16x2_puts("                ");
	}
}
